...
LINUX_HEADERS_CUSTOM_LOCAL_DIRECTORY = $(call qstrip,$(BR2_LINUX_KERNEL_CUSTOM_LOCAL_DIRECTORY))
LINUX_HEADERS_CUSTOM_LOCAL_DIRECTORY_LOCATION = $(call qstrip,$(BR2_LINUX_KERNEL_CUSTOM_LOCAL_DIRECTORY_LOCATION))
...
LINUX_HEADERS_CUSTOM_LOCAL_DIRECTORY =
LINUX_HEADERS_CUSTOM_LOCAL_DIRECTORY_LOCATION =
...
else ifeq ($(LINUX_HEADERS_CUSTOM_LOCAL_DIRECTORY),y)
LINUX_HEADERS_SITE = $(LINUX_HEADERS_CUSTOM_LOCAL_DIRECTORY_LOCATION)
LINUX_HEADERS_SITE_METHOD = local
# Since we're using a local directory, ensure that it exists and is valid.
ifeq ($(BR_BUILDING),y)
ifeq ($(LINUX_HEADERS_SITE),)
$(error No kernel headers local directory set, check your BR2_LINUX_KERNEL_CUSTOM_LOCAL_DIRECTORY_LOCATION setting)
endif
ifneq ("$(wildcard $(LINUX_HEADERS_SITE))","")
# If the directory exists, do nothing more.
else
$(error Local kernel headers directory does not exist: $(LINUX_HEADERS_SITE))
endif
endif # BR_BUILDING
